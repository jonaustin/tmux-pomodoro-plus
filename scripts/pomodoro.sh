...
get_pomodoro_duration() {
  local dur=$(get_tmux_option "$pomodoro_duration_minutes" "25")
  echo $(($dur * 60))
}

get_pomodoro_break() {
  local dur=$(get_tmux_option "$pomodoro_break_minutes" "5")
  echo $(($dur * 60))
}
    ...
    pomodoro_menu() {
      ...
      pomodoro_status() {
        ...
        if [ $pomodoro_start_time -eq -1 ]; then
          ...
          printf "$(get_tmux_option "$pomodoro_complete" \
            "$pomodoro_complete_default")$((-($difference - $(get_pomodoro_duration) - $(get_pomodoro_break))))s "
        else
          local seconds_left=$(($(get_pomodoro_duration) - $difference))
          local minutes_1_precision_left=$(awk -v n=$seconds_left 'BEGIN {printf "%.1f\n", (n/60.0)}')
          printf "$(get_tmux_option "$pomodoro_on" "$pomodoro_on_default") ${minutes_1_precision_left}m "
        fi
      }
